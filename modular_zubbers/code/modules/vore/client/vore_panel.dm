...
/mob
	var/datum/vore_panel/vorePanel
...
/mob/proc/insidePanel()
	...
	if(!vorePanel)
		if(!isnewplayer(src))
			warning("[src] ([type], \ref[src]) didn't have a vorePanel and tried to use the verb.")
		vorePanel = new(src)
...
/datum/vore_panel/New(mob/new_host)
	if(istype(new_host))
		host = new_host
	. = ..()

/datum/vore_panel/Destroy()
	host = null
	. = ..()

// TODO: assets
// /datum/vore_panel/ui_assets(mob/user)
// 	. = ..()
...
/datum/vore_panel/ui_interact(mob/user, datum/tgui/ui)
...
/datum/vore_panel/ui_host(mob/user)
	return host

// Note, in order to allow others to look at others vore panels, this state would need
// to be modified.
/datum/vore_panel/ui_state(mob/user)
	return GLOB.vorepanel_state

/datum/vore_panel/var/static/list/nom_icons
/datum/vore_panel/proc/cached_nom_icon(atom/target)
	...
	if(isobj(target))
		...
		var/mob/M = target
		if(istype(M,/mob/living/simple_animal)) //CHOMPedit: not generating unique icons for every simplemob(number)
...
/datum/vore_panel/ui_data(mob/user)
	var/list/data = list()
	...
	if(isbelly(hostloc))
		...
		if(host.vore_info.absorbed && inside_belly.absorbed_desc)
		...
		if((host.vore_info.absorbed && inside_belly.absorbed_desc) || (inside_belly.desc))
			var/formatted_desc
		...
		inside = list(
			"absorbed" = host.vore_info.absorbed,
		...
		for(var/atom/movable/O in inside_belly)
			...
			if(isliving(O))
				var/mob/living/M = O
				info["stat"] = M.stat
				if(M.vore_info.absorbed)
					info["absorbed"] = TRUE
			inside_contents.Add(list(info))
		inside["contents"] = inside_contents
	data["inside"] = inside

	// TODO: robots and simple vore animals
	// var/is_cyborg = FALSE
	// var/is_vore_simple_animal = FALSE
	// if(isrobot(host))
	// 	is_cyborg = TRUE
	// else if(istype(host, /mob/living/simple_animal/vore))	//So far, this does nothing. But, creating this for future belly work
	// 	is_vore_simple_animal = TRUE
	// data["host_mobtype"] = list(
	// 	"is_cyborg" = is_cyborg,
	// 	"is_vore_simple_animal" = is_vore_simple_animal
	// )

	var/list/our_bellies = list()
	for(var/obj/belly/B as anything in host.vore_info.vore_organs)
		our_bellies.Add(list(list(
			"selected" = (B == host.vore_info.vore_selected),
			"name" = B.name,
			"ref" = "\ref[B]",
	...
	if(host.vore_info.vore_selected)
		var/obj/belly/selected = host.vore_info.vore_selected
		selected_list = list(
			...
			"nutrition_ex" = host.vore_info.nutrition_message_visible,
			"weight_ex" = host.vore_info.weight_message_visible,
			...
			"belly_sprite_option_shown" = LAZYLEN(host.vore_info.vore_icon_bellies) >= 1 ? TRUE : FALSE,
			// TODO: tf/tails
		...
		if(selected.colorization_enabled)
			selected_list["possible_fullscreens"] = icon_states('modular_zubbers/icons/vore/screen_full_vore_ch.dmi') //Makes any icons inside of here selectable. //CHOMPedit
		else
			selected_list["possible_fullscreens"] = icon_states('modular_zubbers/icons/vore/screen_full_vore.dmi') //Where all upstream stomachs are stored. I'm not touching the chunks of comments below but they are inaccurate here.
	...
	data["prefs"] = list(
		"digestable" = host.vore_info.digestable,
		"devourable" = host.vore_info.devourable,
		"resizable" = host.vore_info.resizable,
		"feeding" = host.vore_info.feeding,
		"absorbable" = host.vore_info.absorbable,
		"digest_leave_remains" = host.vore_info.digest_leave_remains,
		"allowmobvore" = host.vore_info.allowmobvore,
		"permit_healbelly" = host.vore_info.permit_healbelly,
		"show_vore_fx" = host.vore_info.show_vore_fx,
		"can_be_drop_prey" = host.vore_info.can_be_drop_prey,
		"can_be_drop_pred" = host.vore_info.can_be_drop_pred,
		"latejoin_vore" = host.vore_info.latejoin_vore, //CHOMPedit
		"latejoin_prey" = host.vore_info.latejoin_prey, //CHOMPedit
		"allow_spontaneous_tf" = host.vore_info.allow_spontaneous_tf,
		"step_mechanics_active" = host.vore_info.step_mechanics_pref,
		"pickup_mechanics_active" = host.vore_info.pickup_pref,
		"noisy" = host.vore_info.noisy,
		//CHOMPedit start, liquid belly prefs
		"liq_rec" = host.vore_info.receive_reagents,
		"liq_giv" = host.vore_info.give_reagents,
		// TODO: autotransferring
		// "autotransferable" = host.vore_info.autotransferable,
		"noisy_full" = host.vore_info.noisy_full, //Belching while full
		//CHOMPedit end
		"drop_vore" = host.vore_info.drop_vore,
		"slip_vore" = host.vore_info.slip_vore,
		"stumble_vore" = host.vore_info.stumble_vore,
		"throw_vore" = host.vore_info.throw_vore,
		"food_vore" = host.vore_info.food_vore,
		"nutrition_message_visible" = host.vore_info.nutrition_message_visible,
		"nutrition_messages" = host.vore_info.nutrition_messages,
		"weight_message_visible" = host.vore_info.weight_message_visible,
		"weight_messages" = host.vore_info.weight_messages,
		"eating_privacy_global" = host.vore_info.eating_privacy_global,
		//CHOMPEdit start, vore sprites
		"vore_sprite_color" = host.vore_info.vore_sprite_color,
		"vore_sprite_multiply" = host.vore_info.vore_sprite_multiply
		//CHOMPEdit end
	)

	return data

/datum/vore_panel/ui_act(action, params)
	if(..())
		return TRUE
	...
	switch(action)
		...
		if("newbelly")
			if(host.vore_info.vore_organs.len >= BELLIES_MAX)
				return FALSE
			...
			NB.name = new_name
			host.vore_info.vore_selected = NB
		...
		if("importpanel")
			...
			for(var/list/raw_list in input_data)
				...
				for(var/obj/belly/B in host.vore_info.vore_organs)
					if(lowertext(B.name) == lowertext(raw_list["name"]))
						updated += raw_list["name"]
						break
				if(!pickOne && length(host.vore_info.vore_organs)+length(valid_names)-length(updated) >= BELLIES_MAX) continue
			...
			for(var/list/belly_data in valid_lists)
				...
				for(var/obj/belly/existing_belly in host.vore_info.vore_organs)
					if(lowertext(existing_belly.name) == lowertext(belly_data["name"]))
						new_belly = existing_belly
						break
				if(!new_belly && length(host.vore_info.vore_organs) < BELLIES_MAX)
					new_belly = new(host)
				...
				if(istext(belly_data["belly_sprite_to_affect"]))
					...
					if(new_belly_sprite_to_affect)
						if (new_belly_sprite_to_affect in host.vore_info.vore_icon_bellies)
							new_belly.belly_sprite_to_affect = new_belly_sprite_to_affect

				// TODO: undergarment_chosen
				...
				*/
				...
				var/possible_fullscreens = icon_states('modular_zubbers/icons/vore/screen_full_vore_ch.dmi')
				if(!new_belly.colorization_enabled)
					possible_fullscreens = icon_states('modular_zubbers/icons/vore/screen_full_vore.dmi')
		...
		//CHOMPAdd End
		if("bellypick")
			host.vore_info.vore_selected = locate(params["bellypick"])
			return TRUE
		if("move_belly")
			var/dir = text2num(params["dir"])
			if(LAZYLEN(host.vore_info.vore_organs) <= 1)
				to_chat(usr, "<span class='warning'>You can't sort bellies with only one belly to sort...</span>")
				return TRUE

			var/current_index = host.vore_info.vore_organs.Find(host.vore_info.vore_selected)
			if(current_index)
				var/new_index = clamp(current_index + dir, 1, LAZYLEN(host.vore_info.vore_organs))
				host.vore_info.vore_organs.Swap(current_index, new_index)
				unsaved_changes = TRUE
			return TRUE
		...
		// 	var/datum/vore_panel/export_panel/exportPanel
		// 	if(!exportPanel)
		// 		exportPanel = new(usr)
		...
		//CHOMPEdit End
		if("setflavor")
			var/new_flavor = html_encode(tgui_input_text(usr,"What your character tastes like (400ch limit). This text will be printed to the pred after 'X tastes of...' so just put something like 'strawberries and cream':","Character Flavor",host.vore_info.vore_taste))
			if(!new_flavor)
				return FALSE
			...
			if(length(new_flavor) > FLAVOR_MAX)
				...
				return FALSE
			host.vore_info.vore_taste = new_flavor
			unsaved_changes = TRUE
			return TRUE
		if("setsmell")
			var/new_smell = html_encode(tgui_input_text(usr,"What your character smells like (400ch limit). This text will be printed to the pred after 'X smells of...' so just put something like 'strawberries and cream':","Character Smell",host.vore_info.vore_smell))
			if(!new_smell)
				return FALSE
			...
			if(length(new_smell) > FLAVOR_MAX)
				...
				return FALSE
			host.vore_info.vore_smell = new_smell
			unsaved_changes = TRUE
			return TRUE
		if("toggle_dropnom_pred")
			host.vore_info.can_be_drop_pred = !host.vore_info.can_be_drop_pred
			if(host.client.prefs_vr)
				host.client.prefs_vr.can_be_drop_pred = host.vore_info.can_be_drop_pred
			unsaved_changes = TRUE
			return TRUE
		if("toggle_dropnom_prey")
			host.vore_info.can_be_drop_prey = !host.vore_info.can_be_drop_prey
			if(host.client.prefs_vr)
				host.client.prefs_vr.can_be_drop_prey = host.vore_info.can_be_drop_prey
			unsaved_changes = TRUE
			return TRUE
		if("toggle_latejoin_vore")
			host.vore_info.latejoin_vore = !host.vore_info.latejoin_vore
			if(host.client.prefs_vr)
				host.client.prefs_vr.latejoin_vore = host.vore_info.latejoin_vore
			unsaved_changes = TRUE
			return TRUE
		if("toggle_latejoin_prey")
			host.vore_info.latejoin_prey = !host.vore_info.latejoin_prey
			if(host.client.prefs_vr)
				host.client.prefs_vr.latejoin_prey = host.vore_info.latejoin_prey
			unsaved_changes = TRUE
			return TRUE
		if("toggle_allow_spontaneous_tf")
			host.vore_info.allow_spontaneous_tf = !host.vore_info.allow_spontaneous_tf
			if(host.client.prefs_vr)
				host.client.prefs_vr.allow_spontaneous_tf = host.vore_info.allow_spontaneous_tf
			unsaved_changes = TRUE
			return TRUE
		if("toggle_digest")
			host.vore_info.digestable = !host.vore_info.digestable
			if(host.client.prefs_vr)
				host.client.prefs_vr.digestable = host.vore_info.digestable
			unsaved_changes = TRUE
			return TRUE
		if("toggle_global_privacy")
			host.vore_info.eating_privacy_global = !host.vore_info.eating_privacy_global
			if(host.client.prefs_vr)
				host.vore_info.eating_privacy_global = host.vore_info.eating_privacy_global
			unsaved_changes = TRUE
			return TRUE
		if("toggle_devour")
			host.vore_info.devourable = !host.vore_info.devourable
			if(host.client.prefs_vr)
				host.client.prefs_vr.devourable = host.vore_info.devourable
			unsaved_changes = TRUE
			return TRUE
		if("toggle_resize")
			host.vore_info.resizable = !host.vore_info.resizable
			if(host.client.prefs_vr)
				host.client.prefs_vr.resizable = host.vore_info.resizable
			unsaved_changes = TRUE
			return TRUE
		if("toggle_feed")
			host.vore_info.feeding = !host.vore_info.feeding
			if(host.client.prefs_vr)
				host.client.prefs_vr.feeding = host.vore_info.feeding
			unsaved_changes = TRUE
			return TRUE
		if("toggle_absorbable")
			host.vore_info.absorbable = !host.vore_info.absorbable
			if(host.client.prefs_vr)
				host.client.prefs_vr.absorbable = host.vore_info.absorbable
			unsaved_changes = TRUE
			return TRUE
		if("toggle_leaveremains")
			host.vore_info.digest_leave_remains = !host.vore_info.digest_leave_remains
			if(host.client.prefs_vr)
				host.client.prefs_vr.digest_leave_remains = host.vore_info.digest_leave_remains
			unsaved_changes = TRUE
			return TRUE
		if("toggle_mobvore")
			host.vore_info.allowmobvore = !host.vore_info.allowmobvore
			if(host.client.prefs_vr)
				host.client.prefs_vr.allowmobvore = host.vore_info.allowmobvore
			unsaved_changes = TRUE
			return TRUE
		if("toggle_steppref")
			host.vore_info.step_mechanics_pref = !host.vore_info.step_mechanics_pref
			if(host.client.prefs_vr)
				host.client.prefs_vr.step_mechanics_pref = host.vore_info.step_mechanics_pref
			unsaved_changes = TRUE
			return TRUE
		if("toggle_pickuppref")
			host.vore_info.pickup_pref = !host.vore_info.pickup_pref
			if(host.client.prefs_vr)
				host.client.prefs_vr.pickup_pref = host.vore_info.pickup_pref
			unsaved_changes = TRUE
			return TRUE
		if("toggle_healbelly")
			host.vore_info.permit_healbelly = !host.vore_info.permit_healbelly
			if(host.client.prefs_vr)
				host.client.prefs_vr.permit_healbelly = host.vore_info.permit_healbelly
			unsaved_changes = TRUE
			return TRUE
		if("toggle_fx")
			host.vore_info.show_vore_fx = !host.vore_info.show_vore_fx
			if(host.client.prefs_vr)
				host.client.prefs_vr.show_vore_fx = host.vore_info.show_vore_fx
			// TODO: vore_fx
			...
			if(!host.hud_used.hud_shown)
				host.hud_used.show_hud(HUD_STYLE_STANDARD)
			unsaved_changes = TRUE
			return TRUE
		if("toggle_noisy")
			host.vore_info.noisy = !host.vore_info.noisy
			unsaved_changes = TRUE
			return TRUE
		...
		if("toggle_liq_rec")
			host.vore_info.receive_reagents = !host.vore_info.receive_reagents
			if(host.client.prefs_vr)
				host.client.prefs_vr.receive_reagents = host.vore_info.receive_reagents
			unsaved_changes = TRUE
			return TRUE
		if("toggle_liq_giv")
			host.vore_info.give_reagents = !host.vore_info.give_reagents
			if(host.client.prefs_vr)
				host.client.prefs_vr.give_reagents = host.vore_info.give_reagents
			unsaved_changes = TRUE
			return TRUE
		// TODO: item transfers
		...
		if("toggle_noisy_full")
			host.vore_info.noisy_full = !host.vore_info.noisy_full
			unsaved_changes = TRUE
			return TRUE
		//CHOMPedit end
		if("toggle_drop_vore")
			host.vore_info.drop_vore = !host.vore_info.drop_vore
			unsaved_changes = TRUE
			return TRUE
		if("toggle_slip_vore")
			host.vore_info.slip_vore = !host.vore_info.slip_vore
			unsaved_changes = TRUE
			return TRUE
		if("toggle_stumble_vore")
			host.vore_info.stumble_vore = !host.vore_info.stumble_vore
			unsaved_changes = TRUE
			return TRUE
		if("toggle_throw_vore")
			host.vore_info.throw_vore = !host.vore_info.throw_vore
			unsaved_changes = TRUE
			return TRUE
		if("toggle_food_vore")
			host.vore_info.food_vore = !host.vore_info.food_vore
			unsaved_changes = TRUE
			return TRUE
		if("switch_selective_mode_pref")
			host.vore_info.selective_preference = tgui_input_list(usr, "What would you prefer happen to you with selective bellymode?","Selective Bellymode", list(DM_DEFAULT, DM_DIGEST, DM_ABSORB, DM_DRAIN))
			if(!(host.vore_info.selective_preference))
				host.vore_info.selective_preference = DM_DEFAULT
			if(host.client.prefs_vr)
				host.client.prefs_vr.selective_preference = host.vore_info.selective_preference
			unsaved_changes = TRUE
			return TRUE
		if("toggle_nutrition_ex")
			host.vore_info.nutrition_message_visible = !host.vore_info.nutrition_message_visible
			unsaved_changes = TRUE
			return TRUE
		if("toggle_weight_ex")
			host.vore_info.weight_message_visible = !host.vore_info.weight_message_visible
			unsaved_changes = TRUE
			return TRUE
		//CHOMPEdit start - vore sprites color
		if("set_vs_color")
			var/belly_choice = tgui_input_list(usr, "Which vore sprite are you going to edit the color of?", "Vore Sprite Color", host.vore_info.vore_icon_bellies)
			var/newcolor = input(usr, "Choose a color.", "", host.vore_info.vore_sprite_color[belly_choice]) as color|null
			if(newcolor)
				host.vore_info.vore_sprite_color[belly_choice] = newcolor
				var/multiply = tgui_input_list(usr, "Set the color to be applied multiplicatively or additively? Currently in [host.vore_info.vore_sprite_multiply[belly_choice] ? "Multiply" : "Add"]", "Vore Sprite Color", list("Multiply", "Add"))
				if(multiply == "Multiply")
					host.vore_info.vore_sprite_multiply[belly_choice] = TRUE
				else if(multiply == "Add")
					host.vore_info.vore_sprite_multiply[belly_choice] = FALSE
				// TODO: update body icons
				// host.update_icons_body()
			return TRUE
		//CHOMPEdit end

/datum/vore_panel/proc/pick_from_inside(mob/user, params)
	...
	switch(intent)
		if("Help Out") //Help the inside-mob out
			if(host.stat || host.vore_info.absorbed || M.vore_info.absorbed)
				to_chat(user, "<span class='warning'>You can't do that in your state!</span>")
				return TRUE
		...
		if("Devour") //Eat the inside mob
			if(host.vore_info.absorbed || host.stat)
				to_chat(user,"<span class='warning'>You can't do that in your state!</span>")
				return TRUE
			...
			var/obj/belly/TB = host.vore_info.vore_selected
			...
			if((host in OB) && (M in OB)) //Make sure they're still here.
				...
				if(M.vore_info.absorbed)
					M.vore_info.absorbed = FALSE
					OB.handle_absorb_langs(M, OB.owner)
				TB.nom_mob(M)

/datum/vore_panel/proc/pick_from_outside(mob/user, params)
	var/intent
	...
	if(params["pickall"])
		...
		switch(intent)
			...
			if("Eject all")
				...
				host.vore_info.vore_selected.release_all_contents()
				return TRUE
			...
			if("Move all")
				...
				var/obj/belly/choice = tgui_input_list(user, "Move all where?","Select Belly", host.vore_info.vore_organs)
				if(!choice)
					return FALSE

				for(var/atom/movable/target in host.vore_info.vore_selected)
					to_chat(target,"<span class='warning'>You're squished from [host]'s [lowertext(host.vore_info.vore_selected)] to their [lowertext(choice.name)]!</span>")
					host.vore_info.vore_selected.transfer_contents(target, choice, 1)
				return TRUE
		return

	var/atom/movable/target = locate(params["pick"])
	if(!(target in host.vore_info.vore_selected))
	...
	switch(intent)
		...
		if("Eject")
			...
			host.vore_info.vore_selected.release_specific_contents(target)
			return TRUE
		...
		if("Move")
			if(host.stat)
				to_chat(user,"<span class='warning'>You can't do that in your state!</span>")
				return TRUE
			var/obj/belly/choice = tgui_input_list(usr, "Move [target] where?","Select Belly", host.vore_info.vore_organs)
			if(!choice || !(target in host.vore_info.vore_selected))
				return TRUE
			to_chat(target,"<span class='warning'>You're squished from [host]'s [lowertext(host.vore_info.vore_selected.name)] to their [lowertext(choice.name)]!</span>")
			host.vore_info.vore_selected.transfer_contents(target, choice)
		...
		if("Transfer")
			...
			for(var/mob/living/candidate in range(1, host))
				if(istype(candidate) && !(candidate == host))
					if(candidate.vore_info.vore_organs.len && candidate.vore_info.feeding && !candidate.vore_info.no_vore)
			...
			var/obj/belly/choice = tgui_input_list(user, "Move [target] where?","Select Belly", belly_owner.vore_info.vore_organs)
			if(!choice || !(target in host.vore_info.vore_selected) || !belly_owner || !(belly_owner in range(1, host)))
				return TRUE

			if(belly_owner != host)
				to_chat(user, "<span class='notice'>Transfer offer sent. Await their response.</span>")
				var/accepted = tgui_alert(belly_owner, "[host] is trying to transfer [target] from their [lowertext(host.vore_info.vore_selected.name)] into your [lowertext(choice.name)]. Do you accept?", "Feeding Offer", list("Yes", "No"))
				if(accepted != "Yes")
					to_chat(user, "<span class='warning'>[belly_owner] refused the transfer!!</span>")
					return TRUE
				if(!belly_owner || !(belly_owner in range(1, host)))
					return TRUE
				to_chat(target,"<span class='warning'>You're squished from [host]'s [lowertext(host.vore_info.vore_selected.name)] to [belly_owner]'s [lowertext(choice.name)]!</span>")
				to_chat(belly_owner,"<span class='warning'>[target] is squished from [host]'s [lowertext(host.vore_info.vore_selected.name)] to your [lowertext(choice.name)]!</span>")
				host.vore_info.vore_selected.transfer_contents(target, choice)
			else
				to_chat(target,"<span class='warning'>You're squished from [host]'s [lowertext(host.vore_info.vore_selected.name)] to their [lowertext(choice.name)]!</span>")
				host.vore_info.vore_selected.transfer_contents(target, choice)
		...
		// 		var/accepted = tgui_alert(T, "[host] is trying to reform your body! Would you like to get reformed inside [host]'s [lowertext(host.vore_info.vore_selected.name)]?", "Reforming Attempt", list("Yes", "No"))
		// 		if(accepted != "Yes")
		...
		// 			T.body_backup = null
		// 			host.vore_info.vore_selected.release_specific_contents(T, TRUE)
		...
		// 			to_chat(user,"<span class='warning'>They don't seem to be reformable!</span>")
		// 			return TRUE
		// 		var/accepted = tgui_alert(MMI.brainmob, "[host] is trying to reform your body! Would you like to get reformed inside [host]'s [lowertext(host.vore_info.vore_selected.name)]?", "Reforming Attempt", list("Yes", "No"))
		// 		if(accepted != "Yes")
		...
		if("Process")
			...
			if(ourtarget.vore_info.digestable)
				process_options += "Digest"

			if(ourtarget.vore_info.absorbable)
			...
			switch(ourchoice)
				if("Digest")
					if(ourtarget.vore_info.absorbed)
					...
					if(isliving(usr))
						...
						// TODO: human nutrition modifiers
						// if(ishuman(l))
...
/datum/vore_panel/proc/set_attr(mob/user, params)
	if(!host.vore_info.vore_selected)
	...
	switch(attr)
		if("b_name")
			...
			host.vore_info.vore_selected.name = new_name
			. = TRUE
		if("b_wetness")
			host.vore_info.vore_selected.is_wet = !host.vore_info.vore_selected.is_wet
			. = TRUE
		if("b_wetloop")
			host.vore_info.vore_selected.wet_loop = !host.vore_info.vore_selected.wet_loop
			. = TRUE
		if("b_mode")
			var/list/menu_list = host.vore_info.vore_selected.digest_modes.Copy()
			var/new_mode = tgui_input_list(usr, "Choose Mode (currently [host.vore_info.vore_selected.digest_mode])", "Mode Choice", menu_list)
			if(!new_mode)
				return FALSE

			host.vore_info.vore_selected.digest_mode = new_mode
			host.vore_info.vore_selected.updateVRPanels()
			. = TRUE
		if("b_addons")
			var/list/menu_list = host.vore_info.vore_selected.mode_flag_list.Copy()
			var/toggle_addon = tgui_input_list(usr, "Toggle Addon", "Addon Choice", menu_list)
			if(!toggle_addon)
				return FALSE
			host.vore_info.vore_selected.mode_flags ^= host.vore_info.vore_selected.mode_flag_list[toggle_addon]
			host.vore_info.vore_selected.items_preserved.Cut() //Re-evaltuate all items in belly on
			host.vore_info.vore_selected.slow_digestion = FALSE //CHOMPAdd Start
			if(host.vore_info.vore_selected.mode_flags & DM_FLAG_SLOWBODY)
				host.vore_info.vore_selected.slow_digestion = TRUE
			if(toggle_addon == "TURBO MODE")
				STOP_PROCESSING(SSbellies, host.vore_info.vore_selected)
				STOP_PROCESSING(SSobj, host.vore_info.vore_selected)
				if(host.vore_info.vore_selected.mode_flags & DM_FLAG_TURBOMODE)
					host.vore_info.vore_selected.speedy_mob_processing = TRUE
					START_PROCESSING(SSobj, host.vore_info.vore_selected)
					...
					host.vore_info.vore_selected.speedy_mob_processing = FALSE
					START_PROCESSING(SSbellies, host.vore_info.vore_selected)
					to_chat(usr, "<span class= 'warning'>TURBO MODE deactivated. Belly processing returned to normal speed.</span>")//CHOMPAdd End
			. = TRUE
		if("b_item_mode")
			var/list/menu_list = host.vore_info.vore_selected.item_digest_modes.Copy()

			var/new_mode = tgui_input_list(usr, "Choose Mode (currently [host.vore_info.vore_selected.item_digest_mode])", "Mode Choice", menu_list)
			if(!new_mode)
				return FALSE

			host.vore_info.vore_selected.item_digest_mode = new_mode
			host.vore_info.vore_selected.items_preserved.Cut() //Re-evaltuate all items in belly on belly-mode change
			. = TRUE
		if("b_contaminates") // CHOMPedit: Reverting upstream's change because why reset save files due to a different server's drama?
			host.vore_info.vore_selected.contaminates = !host.vore_info.vore_selected.contaminates
			. = TRUE
		// TODO: contamination
		// if("b_contamination_flavor")
		// 	var/list/menu_list = contamination_flavors.Copy()
		// 	var/new_flavor = tgui_input_list(usr, "Choose Contamination Flavor Text Type (currently [host.vore_info.vore_selected.contamination_flavor])", "Flavor Choice", menu_list)
		// 	if(!new_flavor)
		// 		return FALSE
		// 	host.vore_info.vore_selected.contamination_flavor = new_flavor
		// 	. = TRUE
		// if("b_contamination_color")
		// 	var/list/menu_list = contamination_colors.Copy()
		// 	var/new_color = tgui_input_list(usr, "Choose Contamination Color (currently [host.vore_info.vore_selected.contamination_color])", "Color Choice", menu_list)
		// 	if(!new_color)
		// 		return FALSE
		// 	host.vore_info.vore_selected.contamination_color = new_color
		// 	host.vore_info.vore_selected.items_preserved.Cut() //To re-contaminate for new color
		// 	. = TRUE
		// TODO: egg
		// if("b_egg_type")
		// 	var/list/menu_list = global_vore_egg_types.Copy()
		// 	var/new_egg_type = tgui_input_list(usr, "Choose Egg Type (currently [host.vore_info.vore_selected.egg_type])", "Egg Choice", menu_list)
		// 	if(!new_egg_type)
		// 		return FALSE
		// 	host.vore_info.vore_selected.egg_type = new_egg_type
		// 	. = TRUE
		...
		// 		return FALSE
		// 	host.vore_info.vore_selected.egg_name = new_egg_name
		// 	. = TRUE
		if("b_recycling")
			host.vore_info.vore_selected.recycling = !host.vore_info.vore_selected.recycling
			. = TRUE//CHOMPAdd End
		if("b_desc")
			var/new_desc = html_encode(tgui_input_text(usr,"Belly Description, '%pred' will be replaced with your name. '%prey' will be replaced with the prey's name. '%belly' will be replaced with your belly's name. ([BELLIES_DESC_MAX] char limit):","New Description",host.vore_info.vore_selected.desc, multiline = TRUE))
		...
		if("b_absorbed_desc")
			...
			if(new_desc)
				...
				if(length(new_desc) > BELLIES_DESC_MAX)
					tgui_alert_async(usr, "Entered belly desc too long. [BELLIES_DESC_MAX] character limit.","Error")
					return FALSE
				host.vore_info.vore_selected.absorbed_desc = new_desc
				. = TRUE
		if("b_msgs")
			...
			switch(params["msgtype"])
				if("dmp")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey when they expire. Write them in 2nd person ('you feel X'). Avoid using %prey in this type."+help,"Digest Message (to prey)",host.vore_info.vore_selected.get_messages("dmp"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"dmp")

				if("dmo")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to you when prey expires in you. Write them in 2nd person ('you feel X'). Avoid using %pred in this type."+help,"Digest Message (to you)",host.vore_info.vore_selected.get_messages("dmo"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"dmo")

				if("amp")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey when their absorption finishes. Write them in 2nd person ('you feel X'). Avoid using %prey in this type. %count will not work for this type, and %countprey will only count absorbed victims."+help,"Digest Message (to prey)",host.vore_info.vore_selected.get_messages("amp"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"amp")

				if("amo")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to you when prey's absorption finishes. Write them in 2nd person ('you feel X'). Avoid using %pred in this type. %count will not work for this type, and %countprey will only count absorbed victims."+help,"Digest Message (to you)",host.vore_info.vore_selected.get_messages("amo"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"amo")

				if("uamp")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey when their unnabsorption finishes. Write them in 2nd person ('you feel X'). Avoid using %prey in this type. %count will not work for this type, and %countprey will only count absorbed victims."+help,"Digest Message (to prey)",host.vore_info.vore_selected.get_messages("uamp"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"uamp")

				if("uamo")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to you when prey's unabsorption finishes. Write them in 2nd person ('you feel X'). Avoid using %pred in this type. %count will not work for this type, and %countprey will only count absorbed victims."+help,"Digest Message (to you)",host.vore_info.vore_selected.get_messages("uamo"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"uamo")

				if("smo")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to those nearby when prey struggles. Write them in 3rd person ('X's Y bulges')."+help,"Struggle Message (outside)",host.vore_info.vore_selected.get_messages("smo"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"smo")

				if("smi")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey when they struggle. Write them in 2nd person ('you feel X'). Avoid using %prey in this type."+help,"Struggle Message (inside)",host.vore_info.vore_selected.get_messages("smi"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"smi")

				if("asmo")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to those nearby when absorbed prey struggles. Write them in 3rd person ('X's Y bulges'). %count will not work for this type, and %countprey will only count absorbed victims."+help,"Struggle Message (outside)",host.vore_info.vore_selected.get_messages("asmo"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"asmo")

				if("asmi")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to absorbed prey when they struggle. Write them in 2nd person ('you feel X'). Avoid using %prey in this type. %count will not work for this type, and %countprey will only count absorbed victims."+help,"Struggle Message (inside)",host.vore_info.vore_selected.get_messages("asmi"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"asmi")

				if("em")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to people who examine you when this belly has contents. Write them in 3rd person ('Their %belly is bulging')."+help,"Examine Message (when full)",host.vore_info.vore_selected.get_messages("em"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"em")

				if("ema")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to people who examine you when this belly has absorbed victims. Write them in 3rd person ('Their %belly is larger'). %count will not work for this type, and %countprey will only count absorbed victims."+help,"Examine Message (with absorbed victims)",host.vore_info.vore_selected.get_messages("ema"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"ema")
				...
				if("en")
					...
					if(index && index <= 10)
						...
						switch(alert)
							if("Clear")
								host.vore_info.nutrition_messages[index] = ""
							if("Edit")
								var/new_message = sanitize(tgui_input_text(user, "Input a message", "Input", host.vore_info.nutrition_messages[index], multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
								if(new_message)
									host.vore_info.nutrition_messages[index] = new_message
				...
				if("ew")
					...
					if(index && index <= 10)
						...
						switch(alert)
							if("Clear")
								host.vore_info.weight_messages[index] = ""
							if("Edit")
								var/new_message = sanitize(tgui_input_text(user, "Input a message", "Input", host.vore_info.weight_messages[index], multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
								if(new_message)
									host.vore_info.weight_messages[index] = new_message

				if("im_digest")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey every minute when you are on Digest mode. Write them in 2nd person ('%pred's %belly squishes down on you.')."+help,"Idle Message (Digest)",host.vore_info.vore_selected.get_messages("im_digest"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"im_digest")

				if("im_hold")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey every minute when you are on Hold mode. Write them in 2nd person ('%pred's %belly squishes down on you.')"+help,"Idle Message (Hold)",host.vore_info.vore_selected.get_messages("im_hold"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"im_hold")

				if("im_holdabsorbed")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey every minute when you are absorbed. Write them in 2nd person ('%pred's %belly squishes down on you.') %count will not work for this type, and %countprey will only count absorbed victims."+help,"Idle Message (Hold Absorbed)",host.vore_info.vore_selected.get_messages("im_holdabsorbed"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"im_holdabsorbed")

				if("im_absorb")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey every minute when you are on Absorb mode. Write them in 2nd person ('%pred's %belly squishes down on you.')"+help,"Idle Message (Absorb)",host.vore_info.vore_selected.get_messages("im_absorb"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"im_absorb")

				if("im_heal")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey every minute when you are on Heal mode. Write them in 2nd person ('%pred's %belly squishes down on you.')"+help,"Idle Message (Heal)",host.vore_info.vore_selected.get_messages("im_heal"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"im_heal")

				if("im_drain")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey every minute when you are on Drain mode. Write them in 2nd person ('%pred's %belly squishes down on you.')"+help,"Idle Message (Drain)",host.vore_info.vore_selected.get_messages("im_drain"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"im_drain")

				if("im_steal")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey every minute when you are on Size Steal mode. Write them in 2nd person ('%pred's %belly squishes down on you.')"+help,"Idle Message (Size Steal)",host.vore_info.vore_selected.get_messages("im_steal"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"im_steal")

				if("im_egg")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey every minute when you are on Encase In Egg mode. Write them in 2nd person ('%pred's %belly squishes down on you.')"+help,"Idle Message (Encase In Egg)",host.vore_info.vore_selected.get_messages("im_egg"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"im_egg")

				if("im_shrink")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey every minute when you are on Shrink mode. Write them in 2nd person ('%pred's %belly squishes down on you.')"+help,"Idle Message (Shrink)",host.vore_info.vore_selected.get_messages("im_shrink"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"im_shrink")

				if("im_grow")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey every minute when you are on Grow mode. Write them in 2nd person ('%pred's %belly squishes down on you.')"+help,"Idle Message (Grow)",host.vore_info.vore_selected.get_messages("im_grow"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"im_grow")

				if("im_unabsorb")
					var/new_message = sanitize(tgui_input_text(user,"These are sent to prey every minute when you are on Unabsorb mode. Write them in 2nd person ('%pred's %belly squishes down on you.')"+help,"Idle Message (Unabsorb)",host.vore_info.vore_selected.get_messages("im_unabsorb"), multiline = TRUE),MAX_MESSAGE_LEN,0,0,0)
					if(new_message)
						host.vore_info.vore_selected.set_messages(new_message,"im_unabsorb")

				if("reset")
					var/confirm = tgui_alert(user,"This will delete any custom messages. Are you sure?","Confirmation",list("Cancel","DELETE"))
					if(confirm == "DELETE")
						host.vore_info.vore_selected.digest_messages_prey = initial(host.vore_info.vore_selected.digest_messages_prey)
						host.vore_info.vore_selected.digest_messages_owner = initial(host.vore_info.vore_selected.digest_messages_owner)
						host.vore_info.vore_selected.absorb_messages_prey = initial(host.vore_info.vore_selected.absorb_messages_prey)
						host.vore_info.vore_selected.absorb_messages_owner = initial(host.vore_info.vore_selected.absorb_messages_owner)
						host.vore_info.vore_selected.unabsorb_messages_prey = initial(host.vore_info.vore_selected.unabsorb_messages_prey)
						host.vore_info.vore_selected.unabsorb_messages_owner = initial(host.vore_info.vore_selected.unabsorb_messages_owner)
						host.vore_info.vore_selected.struggle_messages_outside = initial(host.vore_info.vore_selected.struggle_messages_outside)
						host.vore_info.vore_selected.struggle_messages_inside = initial(host.vore_info.vore_selected.struggle_messages_inside)
						host.vore_info.vore_selected.absorbed_struggle_messages_outside = initial(host.vore_info.vore_selected.absorbed_struggle_messages_outside)
						host.vore_info.vore_selected.absorbed_struggle_messages_inside = initial(host.vore_info.vore_selected.absorbed_struggle_messages_inside)
						host.vore_info.vore_selected.examine_messages = initial(host.vore_info.vore_selected.examine_messages)
						host.vore_info.vore_selected.examine_messages_absorbed = initial(host.vore_info.vore_selected.examine_messages_absorbed)
						host.vore_info.vore_selected.emote_lists = initial(host.vore_info.vore_selected.emote_lists)
			. = TRUE
		if("b_verb")
			...
			host.vore_info.vore_selected.vore_verb = new_verb
			. = TRUE
		if("b_release_verb")
			...
			host.vore_info.vore_selected.release_verb = new_release_verb
			. = TRUE
		if("b_eating_privacy")
			...
			if(privacy_choice == null)
				return FALSE
			host.vore_info.vore_selected.eating_privacy_local = privacy_choice
			. = TRUE
		if("b_silicon_belly")
			...
			if(belly_choice == null)
				return FALSE
			host.vore_info.vore_selected.silicon_belly_overlay_preference = belly_choice
			host.update_icon()
			. = TRUE
		if("b_belly_mob_mult")
			...
			"Set Prey Multiplier", host.vore_info.vore_selected.belly_mob_mult, max_value = 5, min_value = 0)
			if(new_prey_mult == null)
				return FALSE
			host.vore_info.vore_selected.belly_mob_mult = clamp(new_prey_mult, 0, 5) //Max at 5 because in no world will a borg have more than 5 bellies
			host.update_icon()
			. = TRUE
		if("b_belly_item_mult")
			var/new_item_mult = tgui_input_number(user, "Choose the multiplier for items contributing to belly size, \
			ranging from 0 to 10. (Item size affects how much they contribute as well) Set to 0 to disable size checks", "Set Item Multiplier", host.vore_info.vore_selected.belly_item_mult, max_value = 10, min_value = 0)
			if(new_item_mult == null)
				return FALSE
			else
				host.vore_info.vore_selected.belly_item_mult = clamp(new_item_mult, 0, 10) //Max at 10 because items contribute less than mobs, in general
			host.update_icon()
			. = TRUE
		if("b_belly_overall_mult")
			...
			"Set minimum prey amount", host.vore_info.vore_selected.belly_overall_mult, max_value = 5, min_value = 0)
			if(new_overall_mult == null)
				return FALSE
			else
				host.vore_info.vore_selected.belly_overall_mult = clamp(new_overall_mult, 0, 5) // Max at 5 because... no reason to go higher at that point
			host.update_icon()
			. = TRUE
		if("b_fancy_sound")
			host.vore_info.vore_selected.fancy_vore = !host.vore_info.vore_selected.fancy_vore
			host.vore_info.vore_selected.vore_sound = "Gulp"
			host.vore_info.vore_selected.release_sound = "Splatter"
			// defaults as to avoid potential bugs
			. = TRUE
		if("b_release")
			var/choice
			if(host.vore_info.vore_selected.fancy_vore)
				choice = tgui_input_list(user,"Currently set to [host.vore_info.vore_selected.release_sound]","Select Sound", fancy_release_sounds)
			else
				choice = tgui_input_list(user,"Currently set to [host.vore_info.vore_selected.release_sound]","Select Sound", classic_release_sounds)

			if(!choice)
				return FALSE

			host.vore_info.vore_selected.release_sound = choice
			. = TRUE
		if("b_releasesoundtest")
			var/sound/releasetest
			if(host.vore_info.vore_selected.fancy_vore)
				releasetest = fancy_release_sounds[host.vore_info.vore_selected.release_sound]
			else
				releasetest = classic_release_sounds[host.vore_info.vore_selected.release_sound]

			if(releasetest)
				releasetest = sound(releasetest) //CHOMPAdd
				releasetest.volume = host.vore_info.vore_selected.sound_volume //CHOMPAdd
				releasetest.frequency = host.vore_info.vore_selected.noise_freq //CHOMPAdd
				SEND_SOUND(user, releasetest)
			. = TRUE
		if("b_sound")
			var/choice
			if(host.vore_info.vore_selected.fancy_vore)
				choice = tgui_input_list(user,"Currently set to [host.vore_info.vore_selected.vore_sound]","Select Sound", fancy_vore_sounds)
			else
				choice = tgui_input_list(user,"Currently set to [host.vore_info.vore_selected.vore_sound]","Select Sound", classic_vore_sounds)

			if(!choice)
				return FALSE

			host.vore_info.vore_selected.vore_sound = choice
			. = TRUE
		if("b_soundtest")
			var/sound/voretest
			if(host.vore_info.vore_selected.fancy_vore)
				voretest = fancy_vore_sounds[host.vore_info.vore_selected.vore_sound]
			else
				voretest = classic_vore_sounds[host.vore_info.vore_selected.vore_sound]
			if(voretest)
				voretest = sound(voretest) //CHOMPAdd
				voretest.volume = host.vore_info.vore_selected.sound_volume //CHOMPAdd
				voretest.frequency = host.vore_info.vore_selected.noise_freq //CHOMPAdd
		...
		if("b_sound_volume") //CHOMPAdd Start
			...
			if(!isnull(sound_volume_input)) //These have to be 'null' because both cancel and 0 are valid, separate options
				host.vore_info.vore_selected.sound_volume = sanitize_integer(sound_volume_input, 0, 100, initial(host.vore_info.vore_selected.sound_volume))
			. = TRUE
		if("b_noise_freq")
			...
			if(choice == 0)
				host.vore_info.vore_selected.noise_freq = 42500
				return TRUE
			else if(choice == 1)
			...
			host.vore_info.vore_selected.noise_freq = choice
			. = TRUE  //CHOMPAdd End
		if("b_tastes")
			host.vore_info.vore_selected.can_taste = !host.vore_info.vore_selected.can_taste
			. = TRUE
		if("b_feedable") //CHOMPAdd Start
			host.vore_info.vore_selected.is_feedable = !host.vore_info.vore_selected.is_feedable
			. = TRUE
		if("b_entrance_logs")
			host.vore_info.vore_selected.entrance_logs = !host.vore_info.vore_selected.entrance_logs
			. = TRUE
		if("b_item_digest_logs")
			host.vore_info.vore_selected.item_digest_logs = !host.vore_info.vore_selected.item_digest_logs
			. = TRUE //CHOMPAdd End
		if("b_bulge_size")
			...
			if(new_bulge == 0) //Disable.
				host.vore_info.vore_selected.bulge_size = 0
				to_chat(user,"<span class='notice'>Your stomach will not be seen on examine.</span>")
			else if (!ISINRANGE(new_bulge,25,200))
				host.vore_info.vore_selected.bulge_size = 0.25 //Set it to the default.
				to_chat(user,"<span class='notice'>Invalid size.</span>")
			else if(new_bulge)
				host.vore_info.vore_selected.bulge_size = (new_bulge/100)
			. = TRUE
		if("b_display_absorbed_examine")
			host.vore_info.vore_selected.display_absorbed_examine = !host.vore_info.vore_selected.display_absorbed_examine
			. = TRUE
		if("b_grow_shrink")
			var/new_grow = tgui_input_number(user, "Choose the size that prey will be grown/shrunk to, ranging from 25% to 200%", "Set Growth Shrink Size.", host.vore_info.vore_selected.shrink_grow_size, 200, 25)
			if (new_grow == null)
				return FALSE
			if (!ISINRANGE(new_grow,25,200))
				host.vore_info.vore_selected.shrink_grow_size = 1 //Set it to the default
				to_chat(user,"<span class='notice'>Invalid size.</span>")
			else if(new_grow)
				host.vore_info.vore_selected.shrink_grow_size = (new_grow*0.01)
			. = TRUE
		if("b_nutritionpercent")
			var/new_nutrition = tgui_input_number(user, "Choose the nutrition gain percentage you will receive per tick from prey. Ranges from 0.01 to 100.", "Set Nutrition Gain Percentage.", host.vore_info.vore_selected.nutrition_percent, 100, 0.01)
			if(new_nutrition == null)
				return FALSE
			var/new_new_nutrition = clamp(new_nutrition, 0.01, 100)
			host.vore_info.vore_selected.nutrition_percent = new_new_nutrition
			. = TRUE
		if("b_burn_dmg")
			var/new_damage = tgui_input_number(user, "Choose the amount of burn damage prey will take per tick. Ranges from 0 to 6.", "Set Belly Burn Damage.", host.vore_info.vore_selected.digest_burn, 6, 0)
			if(new_damage == null)
				return FALSE
			var/new_new_damage = clamp(new_damage, 0, 6)
			host.vore_info.vore_selected.digest_burn = new_new_damage
			host.vore_info.vore_selected.items_preserved.Cut() //CHOMPAdd
			. = TRUE
		if("b_brute_dmg")
			var/new_damage = tgui_input_number(user, "Choose the amount of brute damage prey will take per tick. Ranges from 0 to 6", "Set Belly Brute Damage.", host.vore_info.vore_selected.digest_brute, 6, 0)
			if(new_damage == null)
				return FALSE
			var/new_new_damage = clamp(new_damage, 0, 6)
			host.vore_info.vore_selected.digest_brute = new_new_damage
			host.vore_info.vore_selected.items_preserved.Cut() //CHOMPAdd
			. = TRUE
		if("b_oxy_dmg")
			var/new_damage = tgui_input_number(user, "Choose the amount of suffocation damage prey will take per tick. Ranges from 0 to 12.", "Set Belly Suffocation Damage.", host.vore_info.vore_selected.digest_oxy, 12, 0)
			if(new_damage == null)
				return FALSE
			var/new_new_damage = clamp(new_damage, 0, 12)
			host.vore_info.vore_selected.digest_oxy = new_new_damage
			. = TRUE
		if("b_tox_dmg")
			var/new_damage = tgui_input_number(user, "Choose the amount of toxins damage prey will take per tick. Ranges from 0 to 6", "Set Belly Toxins Damage.", host.vore_info.vore_selected.digest_tox, 6, 0)
			if(new_damage == null)
				return FALSE
			var/new_new_damage = clamp(new_damage, 0, 6)
			host.vore_info.vore_selected.digest_tox = new_new_damage
			. = TRUE
		if("b_clone_dmg")
			var/new_damage = tgui_input_number(user, "Choose the amount of brute DNA damage (clone) prey will take per tick. Ranges from 0 to 6", "Set Belly Clone Damage.", host.vore_info.vore_selected.digest_clone, 6, 0)
			if(new_damage == null)
				return FALSE
			var/new_new_damage = clamp(new_damage, 0, 6)
			host.vore_info.vore_selected.digest_clone = new_new_damage
			. = TRUE
		if("b_emoteactive")
			host.vore_info.vore_selected.emote_active = !host.vore_info.vore_selected.emote_active
			. = TRUE
		if("b_selective_mode_pref_toggle")
			if(host.vore_info.vore_selected.selective_preference == DM_DIGEST)
				host.vore_info.vore_selected.selective_preference = DM_ABSORB
			else
				host.vore_info.vore_selected.selective_preference = DM_DIGEST
			. = TRUE
		if("b_emotetime")
			var/new_time = tgui_input_number(user, "Choose the period it takes for idle belly emotes to be shown to prey. Measured in seconds, Minimum 1 minute, Maximum 10 minutes.", "Set Belly Emote Delay.", host.vore_info.vore_selected.digest_brute, 600, 60)
			if(new_time == null)
				return FALSE
			var/new_new_time = clamp(new_time, 60, 600)
			host.vore_info.vore_selected.emote_time = new_new_time
			. = TRUE
		if("b_escapable")
			if(host.vore_info.vore_selected.escapable == 0) //Possibly escapable and special interactions.
				host.vore_info.vore_selected.escapable = 1
				to_chat(usr,"<span class='warning'>Prey now have special interactions with your [lowertext(host.vore_info.vore_selected.name)] depending on your settings.</span>")
			else if(host.vore_info.vore_selected.escapable == 1) //Never escapable.
				host.vore_info.vore_selected.escapable = 0
				to_chat(usr,"<span class='warning'>Prey will not be able to have special interactions with your [lowertext(host.vore_info.vore_selected.name)].</span>")
			else
		...
		if("b_escapechance")
			...
			if(!isnull(escape_chance_input)) //These have to be 'null' because both cancel and 0 are valid, separate options
				host.vore_info.vore_selected.escapechance = sanitize_integer(escape_chance_input, 0, 100, initial(host.vore_info.vore_selected.escapechance))
			. = TRUE
		if("b_escapetime")
			...
			if(!isnull(escape_time_input))
				host.vore_info.vore_selected.escapetime = sanitize_integer(escape_time_input*10, 10, 600, initial(host.vore_info.vore_selected.escapetime))
			. = TRUE
		if("b_transferchance")
			...
			if(!isnull(transfer_chance_input))
				host.vore_info.vore_selected.transferchance = sanitize_integer(transfer_chance_input, 0, 100, initial(host.vore_info.vore_selected.transferchance))
			. = TRUE
		if("b_transferlocation")
			var/obj/belly/choice = tgui_input_list(usr, "Where do you want your [lowertext(host.vore_info.vore_selected.name)] to lead if prey resists?","Select Belly", (host.vore_info.vore_organs + "None - Remove" - host.vore_info.vore_selected))
			if(!choice) //They cancelled, no changes
				return FALSE
			else if(choice == "None - Remove")
				host.vore_info.vore_selected.transferlocation = null
			else
				host.vore_info.vore_selected.transferlocation = choice.name
			. = TRUE
		if("b_transferchance_secondary")
			...
			if(!isnull(transfer_secondary_chance_input))
				host.vore_info.vore_selected.transferchance_secondary = sanitize_integer(transfer_secondary_chance_input, 0, 100, initial(host.vore_info.vore_selected.transferchance_secondary))
			. = TRUE
		if("b_transferlocation_secondary")
			var/obj/belly/choice_secondary = tgui_input_list(usr, "Where do you want your [lowertext(host.vore_info.vore_selected.name)] to alternately lead if prey resists?","Select Belly", (host.vore_info.vore_organs + "None - Remove" - host.vore_info.vore_selected))

			if(!choice_secondary) //They cancelled, no changes
				return FALSE
			else if(choice_secondary == "None - Remove")
				host.vore_info.vore_selected.transferlocation_secondary = null
			else
				host.vore_info.vore_selected.transferlocation_secondary = choice_secondary.name
			. = TRUE
		if("b_absorbchance")
			...
			if(!isnull(absorb_chance_input))
				host.vore_info.vore_selected.absorbchance = sanitize_integer(absorb_chance_input, 0, 100, initial(host.vore_info.vore_selected.absorbchance))
			. = TRUE
		if("b_digestchance")
			...
			if(!isnull(digest_chance_input))
				host.vore_info.vore_selected.digestchance = sanitize_integer(digest_chance_input, 0, 100, initial(host.vore_info.vore_selected.digestchance))
			. = TRUE
		if("b_autotransferchance") //CHOMPedit Start
			...
			if(!isnull(autotransferchance_input))
				host.vore_info.vore_selected.autotransferchance = sanitize_integer(autotransferchance_input, 0, 100, initial(host.vore_info.vore_selected.autotransferchance))
			. = TRUE
		if("b_autotransferwait")
			...
			if(!isnull(autotransferwait_input))
				host.vore_info.vore_selected.autotransferwait = sanitize_integer(autotransferwait_input*10, 10, 18000, initial(host.vore_info.vore_selected.autotransferwait))
			. = TRUE
		if("b_autotransferlocation")
			var/obj/belly/choice = tgui_input_list(usr, "Where do you want your [lowertext(host.vore_info.vore_selected.name)] auto-transfer to?","Select Belly", (host.vore_info.vore_organs + "None - Remove" - host.vore_info.vore_selected))
			if(!choice) //They cancelled, no changes
				return FALSE
			else if(choice == "None - Remove")
				host.vore_info.vore_selected.autotransferlocation = null
			else
				host.vore_info.vore_selected.autotransferlocation = choice.name
			. = TRUE
		if("b_autotransferchance_secondary")
			...
			if(!isnull(autotransferchance_secondary_input))
				host.vore_info.vore_selected.autotransferchance_secondary = sanitize_integer(autotransferchance_secondary_input, 0, 100, initial(host.vore_info.vore_selected.autotransferchance_secondary))
			. = TRUE
		if("b_autotransferlocation_secondary")
			var/obj/belly/choice = tgui_input_list(usr, "Where do you want your secondary [lowertext(host.vore_info.vore_selected.name)] auto-transfer to?","Select Belly", (host.vore_info.vore_organs + "None - Remove" - host.vore_info.vore_selected))
			if(!choice) //They cancelled, no changes
				return FALSE
			else if(choice == "None - Remove")
				host.vore_info.vore_selected.autotransferlocation_secondary = null
			else
				host.vore_info.vore_selected.autotransferlocation_secondary = choice.name
			. = TRUE
		if("b_autotransfer_whitelist")
			var/list/menu_list = host.vore_info.vore_selected.autotransfer_flags_list.Copy()
			var/toggle_addon = tgui_input_list(usr, "Toggle Whitelist", "Whitelist Choice", menu_list)
			if(!toggle_addon)
				return FALSE
			host.vore_info.vore_selected.autotransfer_whitelist ^= host.vore_info.vore_selected.autotransfer_flags_list[toggle_addon]
			. = TRUE
		if("b_autotransfer_blacklist")
			...
			if(!toggle_addon)
				return FALSE
			host.vore_info.vore_selected.autotransfer_blacklist ^= host.vore_info.vore_selected.autotransfer_flags_list[toggle_addon]
			. = TRUE
		if("b_autotransfer_secondary_whitelist")
			...
			if(!toggle_addon)
				return FALSE
			host.vore_info.vore_selected.autotransfer_secondary_whitelist ^= host.vore_info.vore_selected.autotransfer_flags_list[toggle_addon]
			. = TRUE
		if("b_autotransfer_secondary_blacklist")
			...
			if(!toggle_addon)
				return FALSE
			host.vore_info.vore_selected.autotransfer_secondary_blacklist ^= host.vore_info.vore_selected.autotransfer_flags_list[toggle_addon]
			. = TRUE
			. = TRUE
		if("b_autotransfer_whitelist_items")
			var/list/menu_list = host.vore_info.vore_selected.autotransfer_flags_list_items.Copy()
			var/toggle_addon = tgui_input_list(usr, "Toggle Whitelist", "Whitelist Choice", menu_list)
			if(!toggle_addon)
				return FALSE
			host.vore_info.vore_selected.autotransfer_whitelist_items ^= host.vore_info.vore_selected.autotransfer_flags_list_items[toggle_addon]
			. = TRUE
		if("b_autotransfer_blacklist_items")
			...
			if(!toggle_addon)
				return FALSE
			host.vore_info.vore_selected.autotransfer_blacklist_items ^= host.vore_info.vore_selected.autotransfer_flags_list_items[toggle_addon]
			. = TRUE
		if("b_autotransfer_secondary_whitelist_items")
			...
			if(!toggle_addon)
				return FALSE
			host.vore_info.vore_selected.autotransfer_secondary_whitelist_items ^= host.vore_info.vore_selected.autotransfer_flags_list_items[toggle_addon]
			. = TRUE
		if("b_autotransfer_secondary_blacklist_items")
			...
			if(!toggle_addon)
				return FALSE
			host.vore_info.vore_selected.autotransfer_secondary_blacklist_items ^= host.vore_info.vore_selected.autotransfer_flags_list_items[toggle_addon]
			. = TRUE
		if("b_autotransfer_min_amount")
			...
			if(!isnull(autotransfer_min_amount_input))
				host.vore_info.vore_selected.autotransfer_min_amount = sanitize_integer(autotransfer_min_amount_input, 0, 100, initial(host.vore_info.vore_selected.autotransfer_min_amount))
			. = TRUE
		if("b_autotransfer_max_amount")
			...
			if(!isnull(autotransfer_max_amount_input))
				host.vore_info.vore_selected.autotransfer_max_amount = sanitize_integer(autotransfer_max_amount_input, 0, 100, initial(host.vore_info.vore_selected.autotransfer_max_amount))
			. = TRUE
		if("b_autotransfer_enabled")
			host.vore_info.vore_selected.autotransfer_enabled = !host.vore_info.vore_selected.autotransfer_enabled
			. = TRUE //CHOMPedit End
		if("b_fullscreen")
			host.vore_info.vore_selected.belly_fullscreen = params["val"]
			host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_disable_hud")
			host.vore_info.vore_selected.disable_hud = !host.vore_info.vore_selected.disable_hud
			. = TRUE
		if("b_colorization_enabled") //ALLOWS COLORIZATION.
			host.vore_info.vore_selected.colorization_enabled = !host.vore_info.vore_selected.colorization_enabled
			host.vore_info.vore_selected.belly_fullscreen = "dark" //This prevents you from selecting a belly that is not meant to be colored and then turning colorization on.
			. = TRUE
		if("b_preview_belly")
			host.vore_info.vore_selected.vore_preview(host) //Gives them the stomach overlay. It fades away after ~2 seconds as human/life.dm removes the overlay if not in a gut.
			. = TRUE
		if("b_clear_preview")
			host.vore_info.vore_selected.clear_preview(host) //Clears the stomach overlay. This is a failsafe but shouldn't occur.
			. = TRUE
		if("b_fullscreen_color")
			var/newcolor = input(usr, "Choose a color.", "", host.vore_info.vore_selected.belly_fullscreen_color) as color|null
			if(newcolor)
				host.vore_info.vore_selected.belly_fullscreen_color = newcolor
				host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_fullscreen_color2")
			var/newcolor2 = input(usr, "Choose a color.", "", host.vore_info.vore_selected.belly_fullscreen_color2) as color|null
			if(newcolor2)
				host.vore_info.vore_selected.belly_fullscreen_color2 = newcolor2
				host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_fullscreen_color3")
			var/newcolor3 = input(usr, "Choose a color.", "", host.vore_info.vore_selected.belly_fullscreen_color3) as color|null
			if(newcolor3)
				host.vore_info.vore_selected.belly_fullscreen_color3 = newcolor3
				host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_fullscreen_color4")
			var/newcolor4 = input(usr, "Choose a color.", "", host.vore_info.vore_selected.belly_fullscreen_color4) as color|null
			if(newcolor4)
				host.vore_info.vore_selected.belly_fullscreen_color4 = newcolor4
				host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_fullscreen_alpha")
			var/newalpha = tgui_input_number(usr, "Set alpha transparency between 0-255", "Vore Alpha",255,255,0,0,1)
			if(newalpha)
				host.vore_info.vore_selected.belly_fullscreen_alpha = newalpha
				host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		/* //Chomp REMOVE - use our solution, not upstream's
		if("b_fullscreen_color_secondary")
			var/newcolor = input(usr, "Choose a color.", "", host.vore_info.vore_selected.belly_fullscreen_color_secondary) as color|null
			if(newcolor)
				host.vore_info.vore_selected.belly_fullscreen_color_secondary = newcolor
			. = TRUE
		if("b_fullscreen_color_trinary")
			var/newcolor = input(usr, "Choose a color.", "", host.vore_info.vore_selected.belly_fullscreen_color_trinary) as color|null
			if(newcolor)
				host.vore_info.vore_selected.belly_fullscreen_color_trinary = newcolor
			. = TRUE
		*/ //Chomp REMOVE - use our solution, not upstream's
		if("b_save_digest_mode")
			host.vore_info.vore_selected.save_digest_mode = !host.vore_info.vore_selected.save_digest_mode
			. = TRUE
		if("b_del")
			var/alert = tgui_alert(usr, "Are you sure you want to delete your [lowertext(host.vore_info.vore_selected.name)]?","Confirmation",list("Cancel","Delete"))
			if(!(alert == "Delete"))
				return FALSE
			...
			for(var/obj/belly/B as anything in host.vore_info.vore_organs)
				if(B.transferlocation == host.vore_info.vore_selected)
					dest_for = B.name
					...
					break
				if(B.transferlocation_secondary == host.vore_info.vore_selected)
					dest_for = B.name
			...
			qdel(host.vore_info.vore_selected)
			host.vore_info.vore_selected = host.vore_info.vore_organs[1]
			. = TRUE
		if("b_vorespawn_blacklist") //CHOMP Addition
			host.vore_info.vore_selected.vorespawn_blacklist = !host.vore_info.vore_selected.vorespawn_blacklist
			. = TRUE
		if("b_belly_sprite_to_affect") //CHOMP Addition
			var/belly_choice = tgui_input_list(usr, "Which belly sprite do you want your [lowertext(host.vore_info.vore_selected.name)] to affect?","Select Region", host.vore_info.vore_icon_bellies)
			if(!belly_choice) //They cancelled, no changes
				return FALSE
			else
				host.vore_info.vore_selected.belly_sprite_to_affect = belly_choice
				host.update_fullness()
			. = TRUE
		if("b_vore_sprite_flags") //CHOMP Addition
			var/list/menu_list = host.vore_info.vore_selected.vore_sprite_flag_list.Copy()
			var/toggle_vs_flag = tgui_input_list(usr, "Toggle Vore Sprite Modes", "Mode Choice", menu_list)
			if(!toggle_vs_flag)
				return FALSE
			host.vore_info.vore_selected.vore_sprite_flags ^= host.vore_info.vore_selected.vore_sprite_flag_list[toggle_vs_flag]
			. = TRUE
		if("b_affects_vore_sprites") //CHOMP Addition
			host.vore_info.vore_selected.affects_vore_sprites = !host.vore_info.vore_selected.affects_vore_sprites
			host.update_fullness()
			. = TRUE
		if("b_count_absorbed_prey_for_sprites") //CHOMP Addition
			host.vore_info.vore_selected.count_absorbed_prey_for_sprite = !host.vore_info.vore_selected.count_absorbed_prey_for_sprite
			host.update_fullness()
			. = TRUE
		if("b_absorbed_multiplier") //CHOMP Addition
			...
			if(!isnull(absorbed_multiplier_input))
				host.vore_info.vore_selected.absorbed_multiplier = clamp(absorbed_multiplier_input, 0.1, 3)
				host.update_fullness()
			. = TRUE
		if("b_count_liquid_for_sprites") //CHOMP Addition
			host.vore_info.vore_selected.count_liquid_for_sprite = !host.vore_info.vore_selected.count_liquid_for_sprite
			host.update_fullness()
			. = TRUE
		if("b_liquid_multiplier") //CHOMP Addition
			...
			if(!isnull(liquid_multiplier_input))
				host.vore_info.vore_selected.liquid_multiplier = clamp(liquid_multiplier_input, 0.1, 10)
				host.update_fullness()
			. = TRUE
		if("b_count_items_for_sprites") //CHOMP Addition
			host.vore_info.vore_selected.count_items_for_sprite = !host.vore_info.vore_selected.count_items_for_sprite
			host.update_fullness()
			. = TRUE
		if("b_item_multiplier") //CHOMP Addition
			...
			if(!isnull(item_multiplier_input))
				host.vore_info.vore_selected.item_multiplier = clamp(item_multiplier_input, 0.1, 10)
				host.update_fullness()
			. = TRUE
		if("b_health_impacts_size") //CHOMP Addition
			host.vore_info.vore_selected.health_impacts_size = !host.vore_info.vore_selected.health_impacts_size
			host.update_fullness()
			. = TRUE
		if("b_resist_animation") //CHOMP Addition
			host.vore_info.vore_selected.resist_triggers_animation = !host.vore_info.vore_selected.resist_triggers_animation
			. = TRUE
		if("b_size_factor_sprites") //CHOMP Addition
			...
			if(!isnull(size_factor_input))
				host.vore_info.vore_selected.size_factor_for_sprite = clamp(size_factor_input, 0.1, 3)
				host.update_fullness()
			. = TRUE
		// TODO: underwear
		// if("b_undergarment_choice") //CHOMP Addition
		// 	var/datum/category_group/underwear/undergarment_choice = tgui_input_list(usr, "Which undergarment do you want to enable when your [lowertext(host.vore_info.vore_selected.name)] is filled?","Select Undergarment Class", global_underwear.categories)
		// 	if(!undergarment_choice) //They cancelled, no changes
		// 		return FALSE
		// 	else
		// 		host.vore_info.vore_selected.undergarment_chosen = undergarment_choice.name
		// 		host.update_fullness()
		// 	. = TRUE
		// if("b_undergarment_if_none") //CHOMP Addition
		// 	var/datum/category_group/underwear/UWC = global_underwear.categories_by_name[host.vore_info.vore_selected.undergarment_chosen]
		// 	var/datum/category_item/underwear/selected_underwear = tgui_input_list(usr, "If no undergarment is equipped, which undergarment style do you want to use?","Select Underwear Style",UWC.items,host.vore_info.vore_selected.undergarment_if_none)
		// 	if(!selected_underwear) //They cancelled, no changes
		// 		return FALSE
		// 	else
		// 		host.vore_info.vore_selected.undergarment_if_none = selected_underwear
		// 		host.update_fullness()
		// 		host.updateVRPanel()
		// if("b_undergarment_color") //CHOMP Addition
		// 	var/newcolor = input(usr, "Choose a color.", "", host.vore_info.vore_selected.undergarment_color) as color|null
		// 	if(newcolor)
		// 		host.vore_info.vore_selected.undergarment_color = newcolor
		// 		host.update_fullness()
		// 	. = TRUE
		// TODO: tails
		// if("b_tail_to_change_to") //CHOMP Addition
		// 	var/tail_choice = tgui_input_list(usr, "Which tail sprite do you want to use when your [lowertext(host.vore_info.vore_selected.name)] is filled?","Select Sprite", global.tail_styles_list)
		// 	if(!tail_choice) //They cancelled, no changes
		// 		return FALSE
		// 	else
		// 		host.vore_info.vore_selected.tail_to_change_to = tail_choice
		// 	. = TRUE
		// if("b_tail_color") // CHOMP Addition
		// 	var/newcolor = input(usr, "Choose tail color.", "", host.vore_info.vore_selected.tail_colouration) as color|null
		// 	if(newcolor)
		// 		host.vore_info.vore_selected.tail_colouration = newcolor
		// 	. = TRUE
		// if("b_tail_color2") // CHOMP Addition
		// 	var/newcolor = input(usr, "Choose tail secondary color.", "", host.vore_info.vore_selected.tail_extra_overlay) as color|null
		// 	if(newcolor)
		// 		host.vore_info.vore_selected.tail_extra_overlay = newcolor
		// 	. = TRUE
		// if("b_tail_color3") // CHOMP Addition
		// 	var/newcolor = input(usr, "Choose tail tertiary color.", "", host.vore_info.vore_selected.tail_extra_overlay2) as color|null
		// 	if(newcolor)
		// 		host.vore_info.vore_selected.tail_extra_overlay2 = newcolor
		// 	. = TRUE
...
/datum/vore_panel/proc/liq_set_attr(mob/user, params)
	if(!host.vore_info.vore_selected)
	...
	switch(attr)
		if("b_show_liq")
			if(!host.vore_info.vore_selected.show_liquids)
				host.vore_info.vore_selected.show_liquids = 1
				to_chat(usr,"<span class='warning'>Your [lowertext(host.vore_info.vore_selected.name)] now has liquid options.</span>")
			else
				host.vore_info.vore_selected.show_liquids = 0
				to_chat(usr,"<span class='warning'>Your [lowertext(host.vore_info.vore_selected.name)] no longer has liquid options.</span>")
			. = TRUE
		if("b_liq_reagent_gen")
			if(!host.vore_info.vore_selected.reagentbellymode) //liquid container adjustments and interactions.
				host.vore_info.vore_selected.reagentbellymode = 1
				to_chat(usr,"<span class='warning'>Your [lowertext(host.vore_info.vore_selected.name)] now has interactions which can produce liquids.</span>")
			else //Doesnt produce liquids
				host.vore_info.vore_selected.reagentbellymode = 0
				to_chat(usr,"<span class='warning'>Your [lowertext(host.vore_info.vore_selected.name)] wont produce liquids, liquids already in your [lowertext(host.vore_info.vore_selected.name)] must be emptied out or removed with purge.</span>")
			. = TRUE
		if("b_liq_reagent_type")
			var/list/menu_list = host.vore_info.vore_selected.reagent_choices.Copy() //Useful if we want to make certain races, synths, borgs, and other things result in additional reagents to produce - Jack
			var/new_reagent = input("Choose Reagent (currently [host.vore_info.vore_selected.reagent_chosen])") as null|anything in menu_list
			if(!new_reagent)
				return FALSE

			host.vore_info.vore_selected.reagent_chosen = new_reagent
			host.vore_info.vore_selected.ReagentSwitch() // For changing variables when a new reagent is chosen
			. = TRUE
		if("b_liq_reagent_name")
			...
			host.vore_info.vore_selected.reagent_name = new_name
			. = TRUE
		if("b_liq_reagent_transfer_verb")
			...
			host.vore_info.vore_selected.reagent_transfer_verb = new_verb
			. = TRUE
		if("b_liq_reagent_nutri_rate")
			host.vore_info.vore_selected.gen_time_display = input(user, "Choose the time it takes to fill the belly from empty state using nutrition.", "Set Liquid Production Time.")  in list("10 minutes","30 minutes","1 hour","3 hours","6 hours","12 hours","24 hours")|null
			switch(host.vore_info.vore_selected.gen_time_display)
				if("10 minutes")
					host.vore_info.vore_selected.gen_time = 0
				if("30 minutes")
					host.vore_info.vore_selected.gen_time = 2
				if("1 hour")
					host.vore_info.vore_selected.gen_time = 5
				if("3 hours")
					host.vore_info.vore_selected.gen_time = 17
				if("6 hours")
					host.vore_info.vore_selected.gen_time = 35
				if("12 hours")
					host.vore_info.vore_selected.gen_time = 71
				if("24 hours")
					host.vore_info.vore_selected.gen_time = 143
				if(null)
					return FALSE
			. = TRUE
		if("b_liq_reagent_capacity")
			var/new_custom_vol = input(user, "Choose the amount of liquid the belly can contain at most. Ranges from 10 to 300.", "Set Custom Belly Capacity.", host.vore_info.vore_selected.custom_max_volume) as num|null
			if(new_custom_vol == null)
				return FALSE
			var/new_new_custom_vol = clamp(new_custom_vol, 10, 300)
			host.vore_info.vore_selected.custom_max_volume = new_new_custom_vol
			. = TRUE
		if("b_liq_sloshing")
			if(!host.vore_info.vore_selected.vorefootsteps_sounds)
				host.vore_info.vore_selected.vorefootsteps_sounds = 1
				to_chat(usr,"<span class='warning'>Your [lowertext(host.vore_info.vore_selected.name)] can now make sounds when you walk around depending on how full you are.</span>")
			else
				host.vore_info.vore_selected.vorefootsteps_sounds = 0
				to_chat(usr,"<span class='warning'>Your [lowertext(host.vore_info.vore_selected.name)] wont make any liquid sounds no matter how full it is.</span>")
			. = TRUE
		if("b_liq_reagent_addons")
			var/list/menu_list = host.vore_info.vore_selected.reagent_mode_flag_list.Copy()
			var/reagent_toggle_addon = input("Toggle Addon") as null|anything in menu_list
			if(!reagent_toggle_addon)
				return FALSE
			host.vore_info.vore_selected.reagent_mode_flags ^= host.vore_info.vore_selected.reagent_mode_flag_list[reagent_toggle_addon]
			. = TRUE
		if("b_liquid_overlay")
			if(!host.vore_info.vore_selected.liquid_overlay)
				host.vore_info.vore_selected.liquid_overlay = 1
				to_chat(usr,"<span class='warning'>Your [lowertext(host.vore_info.vore_selected.name)] now has liquid overlay enabled.</span>")
			else
				host.vore_info.vore_selected.liquid_overlay = 0
				to_chat(usr,"<span class='warning'>Your [lowertext(host.vore_info.vore_selected.name)] no longer has liquid overlay enabled.</span>")
			. = TRUE
		if("b_max_liquid_level")
			var/new_max_liquid_level = input(user, "Set custom maximum liquid level. 0-100%", "Set Custom Max Level.", host.vore_info.vore_selected.max_liquid_level) as num|null
			if(new_max_liquid_level == null)
				return FALSE
			var/new_new_max_liquid_level = clamp(new_max_liquid_level, 0, 100)
			host.vore_info.vore_selected.max_liquid_level = new_new_max_liquid_level
			host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_custom_reagentcolor")
			var/newcolor = input(usr, "Choose custom color for liquid overlay. Cancel for normal reagent color.", "", host.vore_info.vore_selected.custom_reagentcolor) as color|null
			if(newcolor)
				host.vore_info.vore_selected.custom_reagentcolor = newcolor
			else
				host.vore_info.vore_selected.custom_reagentcolor = null
			host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_custom_reagentalpha")
			...
			if(newalpha != null)
				host.vore_info.vore_selected.custom_reagentalpha = newalpha
			else
				host.vore_info.vore_selected.custom_reagentalpha = null
			host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_mush_overlay")
			if(!host.vore_info.vore_selected.mush_overlay)
				host.vore_info.vore_selected.mush_overlay = 1
				to_chat(usr,"<span class='warning'>Your [lowertext(host.vore_info.vore_selected.name)] now has fullness overlay enabled.</span>")
			else
				host.vore_info.vore_selected.mush_overlay = 0
				to_chat(usr,"<span class='warning'>Your [lowertext(host.vore_info.vore_selected.name)] no longer has fullness overlay enabled.</span>")
			host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_mush_color")
			var/newcolor = input(usr, "Choose custom color for mush overlay.", "", host.vore_info.vore_selected.mush_color) as color|null
			if(newcolor)
				host.vore_info.vore_selected.mush_color = newcolor
				host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_mush_alpha")
			var/newalpha = tgui_input_number(usr, "Set alpha transparency between 0-255", "Mush Alpha",255,255,0,0,1)
			if(newalpha != null)
				host.vore_info.vore_selected.mush_alpha = newalpha
				host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_max_mush")
			var/new_max_mush = input(user, "Choose the amount of nutrition required for full mush overlay. Ranges from 0 to 6000. Default 500.", "Set Fullness Overlay Scaling.", host.vore_info.vore_selected.max_mush) as num|null
			if(new_max_mush == null)
				return FALSE
			var/new_new_max_mush = clamp(new_max_mush, 0, 6000)
			host.vore_info.vore_selected.max_mush = new_new_max_mush
			host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_min_mush")
			var/new_min_mush = input(user, "Set custom minimum mush level. 0-100%", "Set Custom Minimum.", host.vore_info.vore_selected.min_mush) as num|null
			if(new_min_mush == null)
				return FALSE
			var/new_new_min_mush = clamp(new_min_mush, 0, 100)
			host.vore_info.vore_selected.min_mush = new_new_min_mush
			host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_item_mush_val")
			var/new_item_mush_val = input(user, "Set how much solid belly contents affect mush level. 0-1000 fullness per item.", "Set Item Mush Value.", host.vore_info.vore_selected.item_mush_val) as num|null
			if(new_item_mush_val == null)
				return FALSE
			var/new_new_item_mush_val = clamp(new_item_mush_val, 0, 1000)
			host.vore_info.vore_selected.item_mush_val = new_new_item_mush_val
			host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_metabolism_overlay")
			if(!host.vore_info.vore_selected.metabolism_overlay)
				host.vore_info.vore_selected.metabolism_overlay = 1
				to_chat(usr,"<span class='warning'>Your [lowertext(host.vore_info.vore_selected.name)] now has ingested metabolism overlay enabled.</span>")
			else
				host.vore_info.vore_selected.metabolism_overlay = 0
				to_chat(usr,"<span class='warning'>Your [lowertext(host.vore_info.vore_selected.name)] no longer has ingested metabolism overlay enabled.</span>")
			host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_metabolism_mush_ratio")
			var/new_metabolism_mush_ratio = input(user, "How much should ingested reagents affect fullness overlay compared to nutrition? Nutrition units per reagent unit. Default 15.", "Set Metabolism Mush Ratio.", host.vore_info.vore_selected.metabolism_mush_ratio) as num|null
			if(new_metabolism_mush_ratio == null)
				return FALSE
			var/new_new_metabolism_mush_ratio = clamp(new_metabolism_mush_ratio, 0, 500)
			host.vore_info.vore_selected.metabolism_mush_ratio = new_new_metabolism_mush_ratio
			host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_max_ingested")
			var/new_max_ingested = input(user, "Choose the amount of reagents within ingested metabolism required for full mush overlay when not using mush overlay option. Ranges from 0 to 6000. Default 500.", "Set Metabolism Overlay Scaling.", host.vore_info.vore_selected.max_ingested) as num|null
			if(new_max_ingested == null)
				return FALSE
			var/new_new_max_ingested = clamp(new_max_ingested, 0, 6000)
			host.vore_info.vore_selected.max_ingested = new_new_max_ingested
			host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_custom_ingested_color")
			var/newcolor = input(usr, "Choose custom color for ingested metabolism overlay. Cancel for reagent-based dynamic blend.", "", host.vore_info.vore_selected.custom_ingested_color) as color|null
			if(newcolor)
				host.vore_info.vore_selected.custom_ingested_color = newcolor
			else
				host.vore_info.vore_selected.custom_ingested_color = null
			host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_custom_ingested_alpha")
			...
			if(newalpha != null)
				host.vore_info.vore_selected.custom_ingested_alpha = newalpha
				host.vore_info.vore_selected.update_internal_overlay()
			. = TRUE
		if("b_liq_purge")
			var/alert = alert("Are you sure you want to delete the liquids in your [lowertext(host.vore_info.vore_selected.name)]?","Confirmation","Delete","Cancel")
			if(!(alert == "Delete"))
				return FALSE
			else
				host.vore_info.vore_selected.reagents.clear_reagents()
			. = TRUE

/datum/vore_panel/proc/liq_set_msg(mob/user, params)
	if(!host.vore_info.vore_selected)
		alert("No belly selected to modify.")
		return FALSE
	...
	switch(attr)
		if("b_show_liq_fullness")
			if(!host.vore_info.vore_selected.show_fullness_messages)
				host.vore_info.vore_selected.show_fullness_messages = 1
				to_chat(usr,"<span class='warning'>Your [lowertext(host.vore_info.vore_selected.name)] now has liquid examination options.</span>")
			else
				host.vore_info.vore_selected.show_fullness_messages = 0
				to_chat(usr,"<span class='warning'>Your [lowertext(host.vore_info.vore_selected.name)] no longer has liquid examination options.</span>")
			. = TRUE
		if("b_liq_msg_toggle1")
			host.vore_info.vore_selected.liquid_fullness1_messages = !host.vore_info.vore_selected.liquid_fullness1_messages
			. = TRUE
		if("b_liq_msg_toggle2")
			host.vore_info.vore_selected.liquid_fullness2_messages = !host.vore_info.vore_selected.liquid_fullness2_messages
			. = TRUE
		if("b_liq_msg_toggle3")
			host.vore_info.vore_selected.liquid_fullness3_messages = !host.vore_info.vore_selected.liquid_fullness3_messages
			. = TRUE
		if("b_liq_msg_toggle4")
			host.vore_info.vore_selected.liquid_fullness4_messages = !host.vore_info.vore_selected.liquid_fullness4_messages
			. = TRUE
		if("b_liq_msg_toggle5")
			host.vore_info.vore_selected.liquid_fullness5_messages = !host.vore_info.vore_selected.liquid_fullness5_messages
			. = TRUE
		if("b_liq_msg1")
			...
			var/new_message = input(user,"These are sent to people who examine you when this belly is 0 to 20% full. Write them in 3rd person ('Their %belly is bulging')."+help,"Liquid Examine Message (0 - 20%)",host.vore_info.vore_selected.get_reagent_messages("full1")) as message
			if(new_message)
				host.vore_info.vore_selected.set_reagent_messages(new_message,"full1")
			. = TRUE
		if("b_liq_msg2")
			alert(user,"Setting abusive or deceptive messages will result in a ban. Consider this your warning. Max 150 characters per message, max 10 messages per topic.","Really, don't.")
			var/help = " Press enter twice to separate messages. '%pred' will be replaced with your name. '%prey' will be replaced with the prey's name. '%belly' will be replaced with your belly's name."

			var/new_message = input(user,"These are sent to people who examine you when this belly is 20 to 40% full. Write them in 3rd person ('Their %belly is bulging')."+help,"Liquid Examine Message (20 - 40%)",host.vore_info.vore_selected.get_reagent_messages("full2")) as message
			if(new_message)
				host.vore_info.vore_selected.set_reagent_messages(new_message,"full2")
			. = TRUE
		if("b_liq_msg3")
			alert(user,"Setting abusive or deceptive messages will result in a ban. Consider this your warning. Max 150 characters per message, max 10 messages per topic.","Really, don't.")
			var/help = " Press enter twice to separate messages. '%pred' will be replaced with your name. '%prey' will be replaced with the prey's name. '%belly' will be replaced with your belly's name."

			var/new_message = input(user,"These are sent to people who examine you when this belly is 40 to 60% full. Write them in 3rd person ('Their %belly is bulging')."+help,"Liquid Examine Message (40 - 60%)",host.vore_info.vore_selected.get_reagent_messages("full3")) as message
			if(new_message)
				host.vore_info.vore_selected.set_reagent_messages(new_message,"full3")
			. = TRUE
		if("b_liq_msg4")
			alert(user,"Setting abusive or deceptive messages will result in a ban. Consider this your warning. Max 150 characters per message, max 10 messages per topic.","Really, don't.")
			var/help = " Press enter twice to separate messages. '%pred' will be replaced with your name. '%prey' will be replaced with the prey's name. '%belly' will be replaced with your belly's name."

			var/new_message = input(user,"These are sent to people who examine you when this belly is 60 to 80% full. Write them in 3rd person ('Their %belly is bulging')."+help,"Liquid Examine Message (60 - 80%)",host.vore_info.vore_selected.get_reagent_messages("full4")) as message
			if(new_message)
				host.vore_info.vore_selected.set_reagent_messages(new_message,"full4")
			. = TRUE
		if("b_liq_msg5")
			alert(user,"Setting abusive or deceptive messages will result in a ban. Consider this your warning. Max 150 characters per message, max 10 messages per topic.","Really, don't.")
			var/help = " Press enter twice to separate messages. '%pred' will be replaced with your name. '%prey' will be replaced with the prey's name. '%belly' will be replaced with your belly's name."

			var/new_message = input(user,"These are sent to people who examine you when this belly is 80 to 100% full. Write them in 3rd person ('Their %belly is bulging')."+help,"Liquid Examine Message (80 - 100%)",host.vore_info.vore_selected.get_reagent_messages("full5")) as message
			if(new_message)
				host.vore_info.vore_selected.set_reagent_messages(new_message,"full5")
			. = TRUE
//CHOMPedit end
