...
/datum/digest_mode/digest/process_mob(obj/belly/B, mob/living/L)
	var/oldstat = L.stat
	//Pref protection!
	if(!L.vore_info.digestable || L.vore_info.absorbed)
		return null

	//Person just died in guts!
	if(L.stat == DEAD)
		if(!L.vore_info.digestion_in_progress) //CHOMPEdit Start
			// TODO: digestion noises toggle
	...
	var/offset = (1 + ((L.vore_info.weight - 137) / 137)) // 130 pounds = .95 140 pounds = 1.02
	var/difference = 1 // B.owner.size_multiplier / L.size_multiplier // TODO: sizeplay
	if(B.health_impacts_size) //CHOMPEdit - Health probably changed so...
		B.owner.update_fullness() //CHOMPEdit - This is run whenever a belly's contents are changed.
...
/datum/digest_mode/absorb/process_mob(obj/belly/B, mob/living/L)
	if(!L.vore_info.absorbable || L.vore_info.absorbed)
		return null
	B.steal_nutrition(L)
	if(L.nutrition < 100)
		B.absorb_living(L)
...
/datum/digest_mode/unabsorb/process_mob(obj/belly/B, mob/living/L)
	if(L.vore_info.absorbed && B.owner.nutrition >= 100)
...
/datum/digest_mode/heal/process_mob(obj/belly/B, mob/living/L)
	...
	// 			if(B.health_impacts_size) //CHOMPEdit - Health probably changed so...
	// 				B.owner.update_fullness() //CHOMPEdit - This is run whenever a belly's contents are changed.
	if(B.owner.nutrition > 90 && (L.health < L.maxHealth) && !issynthetic(H))
...
/datum/digest_mode/selective/process_mob(obj/belly/B, mob/living/L)
	...
	switch(L.vore_info.selective_preference)										// First, we respect prey prefs
		if(DM_DIGEST)
			if(L.vore_info.digestable)
		...
		if(DM_DEFAULT)
			switch(B.selective_preference)								// They don't actually care? Time for our own preference.
				if(DM_DIGEST)
					...
					else if(L.vore_info.absorbable)
				...
				if(DM_ABSORB)
					...
					else if(L.vore_info.digestable)
